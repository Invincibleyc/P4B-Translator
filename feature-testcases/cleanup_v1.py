...
p.wait()
out = p.stdout.read()
t1_cpu_time = float(str(out, encoding = "utf-8").split('\n')[-2].split()[3])
...
echo "integrated ${outdir}/${trimmed}-integrated.p4"
    ...
    p = subprocess.Popen([args.bf4_exec, '--dump-instrumented', outinstr, cleanout], 
        stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=False)
    p.wait()
    out = p.stdout.read()
    end = time.time()
    t2_cpu_time = float(str(out, encoding = "utf-8").split('\n')[-2].split()[3])
    ...
    #     stderr=DEVNULL, stdout=DEVNULL)
    p = subprocess.Popen([args.bf4_exec, '--expand-to', outexpd, outinstr], 
        stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=False)
    p.wait()
    out = p.stdout.read()
    end = time.time()
    # print(str(out, encoding = "utf-8").split('\n')[-2])
    t3_cpu_time = float(str(out, encoding = "utf-8").split('\n')[-2].split()[3])
    ...
    #     stderr=DEVNULL, stdout=DEVNULL)
    # print([args.bf4_exec, '--render-integration', outintegrated,
    #         '--integration-template', int_file, '--render-only', outexpd])
    p = subprocess.Popen([args.bf4_exec, '--render-integration', outintegrated,
            '--integration-template', int_file, '--render-only', outexpd], 
    ...
    # print(str(out, encoding = "utf-8").split('\n')[-2])
    t4_cpu_time = float(str(out, encoding = "utf-8").split('\n')[-2].split()[3])
    # print('done integration in {}ms'.format(int((end - start) * 1000)))

    total_cpu_time = t1_cpu_time+t2_cpu_time+t3_cpu_time+t4_cpu_time
    print('Total CPU Time: '+str(total_cpu_time)+' s')
